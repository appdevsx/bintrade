...
import { authorizationCodeAPI, resendAuthorizationCodeAPI, getLanguageAPI } from "@/services/apiClient/apiClient";
...
export default function Authorization() {
    ...
    const handleResend = async () => {
        ...
        try {
            const response = await resendAuthorizationCodeAPI(token);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await authorizationCodeAPI(token, otp.replace(/ - /g, ""));
            response.data.message.success.forEach((msg) => {
                toast.success(msg);
            });
            router.push("/trading");
                    ...
                    <div className="account-footer section--bg p-8">
                        <form className="account-form" onSubmit={handleSubmit}>
                            <div className="form-group mb-2">
                                <label className="text-[13px] font-medium block mb-1">{t("OTP")}<span>*</span></label>
                            ...
                            <div className="form-group mb-2">
                                {canResend ? (
                                    ...
                                    {t("You can resend the code after")}{" "}
